...
def prune(G, field='significance', percent=None, num_remove=None, significance_level = None):
    """
    ...
    if percent:
        ...
        print (field, threshold_index)
        print ('sorted(G.es[field]): ', sorted(G.es[field]))
        ...
        # deathrow = list(map(lambda e: e.index if e[field] > -log(significance_level) else None, G.es))
        deathrow = [e.index for e in G.es if e[field] > -log(significance_level)]
        # print ('deathrow: ', deathrow)
        G.delete_edges(deathrow)
    return G
